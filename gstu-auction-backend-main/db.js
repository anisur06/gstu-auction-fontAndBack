...
const teamSchema = new mongoose.Schema({
  id: { type: String, default: () => 'team-' + Date.now() },
  name: String,
  code: String,
  managerName: String,
  managerEmail: String,
...
const auctionLedgerSchema = new mongoose.Schema({
  id: { type: String, default: () => 'bid-' + Date.now() },
  timestamp: String,
  playerId: String,
  playerName: String,
  teamId: String,
...
const matchSchema = new mongoose.Schema({
  id: { type: String, default: () => 'm-' + Date.now() },
  homeTeam: String,
  awayTeam: String,
  homeScore: Number,
  awayScore: Number,
...
mongoose.connect(MONGO_URI, {
  ...
  console.log('[MONGODB] Connected to MongoDB database successfully at:', MONGO_URI);
  syncFromMongo();
}).catch((err) => {
...
async function syncFromMongo() {
  ...
  try {
    ...
    if (mongoTeams.length > 0) dbData.teams = mongoTeams.map(t => {
      const o = t.toObject();
      if (!o.id) o.id = o._id ? o._id.toString() : 'team-' + Date.now();
      return o;
    });
    if (mongoPlayers.length > 0) dbData.players = mongoPlayers.map(p => {
      const o = p.toObject();
      if (!o.id) o.id = o._id ? o._id.toString() : 'p-' + Date.now();
      return o;
    });
    if (mongoLedger.length > 0) dbData.auctionLedger = mongoLedger.map(l => {
      const o = l.toObject();
      if (!o.id) o.id = o._id ? o._id.toString() : 'bid-' + Date.now();
      return o;
    });
    if (mongoMatches.length > 0) dbData.matches = mongoMatches.map(m => {
      const o = m.toObject();
      if (!o.id) o.id = o._id ? o._id.toString() : 'm-' + Date.now();
      return o;
    });
